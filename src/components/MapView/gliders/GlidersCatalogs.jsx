...
import { getAllCatalogs, getCatalogByURL, getCatalogsThredds } from '../../../services/ThreddsService';
...
const GlidersCatalogs = ({send}) => {
    ...
    const renderTree = (nodes) => (
        ...
        nodeId={nodes.id}
        label={nodes.name}
        onClick={() => checkTree(nodes.is_file, nodes.id, nodes.url, nodes.url_download, nodes.is_profile)}
    ...
    const checkTree = (is_file, id, url, url_download, is_profile) => {
      (is_file) ?
      send(is_file, url, url_download, is_profile)
      : 
      console.log()
    }
        ...
        <div>
        {
          ...
          (
            <TreeView
              ...
              sx={{ marginTop:"10px", flexGrow: 1, maxWidth: 500, overflow: "auto", fontSize: "0.5rem"}}
            >
